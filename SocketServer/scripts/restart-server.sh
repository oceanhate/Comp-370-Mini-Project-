...
restart_server_on_port() {
    ...
    local cmd="cd '$PROJECT_ROOT' && echo '=== RESTARTED: $server_name (Port $port) ===' && java -cp src $server_class $port"
...
if [ -z "$PORT" ] || [ "$PORT" = "all" ]; then
    ...
    if ! is_server_running 8088; then
        restart_server_on_port 8088 "backup2" "Backup Server 2"
        RESTARTED_COUNT=$((RESTARTED_COUNT + 1))
        sleep 0.5
    else
...
# Start server in a new Terminal window
osascript -e 'tell application "Terminal" to do script "cd '"$PROJECT_ROOT"' && echo \"=== RESTARTED: '$SERVER_NAME' (Port '$PORT') ===\" && java -cp src '$SERVER_CLASS' '$PORT'"'
